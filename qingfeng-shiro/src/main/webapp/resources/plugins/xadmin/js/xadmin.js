...
layui.use(['layer','element','jquery'],function() {
    ...
    $('.container .left_open i').click(function(event) {
        var leftWidthPx = $('.left-nav').css('width');
        var leftWidth = leftWidthPx.substring(0,leftWidthPx.length-2);
        if(leftWidth>=200){
            $('.left-nav .open').click();
    ...
    $('#tab_right').on('click', 'dd', function(event) {
        ...
        if(data_type=='this'){
          ...
          $('.layui-tab-title li[lay-id]').find('.layui-tab-close').click();
        }else if(data_type=='left'){
            var tabArr = [];
            $.each(layui.data('tab_list'), function(index, value) {
                tabArr.push(index);
            });
            var currIndex = tabArr.indexOf(lay_id);
            for(var i=0;i<currIndex;i++){
                $('.layui-tab-title li[lay-id='+tabArr[i]+']').find('.layui-tab-close').click();
            }
            ...
            var currIndex = tabArr.indexOf(lay_id);
            for(var i=currIndex+1;i<=tabArr.length;i++){
                $('.layui-tab-title li[lay-id='+tabArr[i]+']').find('.layui-tab-close').click();
            }
            ...
            var currIndex = tabArr.indexOf(lay_id)+1;
            $('.x-iframe').eq(currIndex).attr("src",$('.x-iframe').eq(currIndex).attr('src'));
        }else if(data_type=='newOpen'){
            var tabArr = [];
            $.each(layui.data('tab_list'), function(index, value) {
                tabArr.push(index);
            });
            var currIndex = tabArr.indexOf(lay_id)+1;
            window.open($('.x-iframe').eq(currIndex).attr('src'));
        }
        $('#tab_right').hide();
        $('#tab_show').hide();
    })

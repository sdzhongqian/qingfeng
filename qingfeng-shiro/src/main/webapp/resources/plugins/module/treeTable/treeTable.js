layui.define(['jquery'], function(exports) {
	...
	tree.prototype.template = function(e) {
		var t = this,
			...
			checkboxThead = e.is_checkbox ? '选择' : '',
			...
			thead = checkbox && '<th style="width:28px;">' + checkboxThead + '</th>';
		o.each(t.data(e, e.data), function(idx, item) {
			...
			o.each(e.cols, function(index, obj) {
				...
				// 标记设置行checkbox选中
				//console.log(o.inArray(item[e.checked.key], e.checked.data));
				//console.log(item.id);
				//console.log(e.checked.data);
				//if(item[e.checked.key]=='221'){console.log(e.checked.data.indexOf(item[e.checked.key]));console.log(o.inArray(item[e.checked.key], e.checked.data));console.log(e.checked.data);}
				if(e.is_checkbox && e.checked && e.checked.data.indexOf(item[e.checked.key]) > -1) {
					is_checked = true;
				}
				// 第一次遍历头部的时候拼接表格头部

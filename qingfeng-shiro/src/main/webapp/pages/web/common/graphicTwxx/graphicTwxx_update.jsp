...
<form class="layui-form" action="" id="form">
    <table width="95%" style="margin: 0 auto">
        ...
        <tr>
            ...
            <td colspan="3"><textarea name="intro" id="intro" lay-verify="required" placeholder="请输入简介" class="layui-textarea">${p.intro}</textarea></td>
        </tr>
        <tr>
            <td width="15%" align="right">
            ...
            <td colspan="3">
                <script type="text/plain" id="richText_content" style="width: 100%;height:500px;">${p.content}</script>
        ...
        <tr>
            ...
            <td colspan="3">
                ...
                <input type="hidden" name="read_num" id="read_num" value="${p.read_num}" />
            </td>
        </tr>
        <tr>
            <td width="15%" align="right">
        ...
        <tr>
            ...
            <td colspan="3">
                <select name="remark" id="remark" lay-verify="" style="height: 32px;" class="layui-input">
                    <option value=""></option>
                    <option <c:if test="${p.remark=='0'}"> selected </c:if> value="0">北京</option>
                    <option <c:if test="${p.remark=='1'}"> selected </c:if> value="1">上海</option>
                    <option <c:if test="${p.remark=='2'}"> selected </c:if> value="2">深圳</option>
                </select>
            </td>
        </tr>
                    ...
                    <c:if test="${fn:length(list)>0}">
                        <c:forEach items="${list}" var="var" varStatus="status">
                            <tr id="child_tr_ls${var.id}">
                                            <td><input type="text" name="name" id="name_1" value="${var.name}" lay-verify="required" autocomplete="off" placeholder="附件名称" class="layui-input"></td>
                                            <td><textarea name="desnames" id="desnames_1" lay-verify="email" placeholder="请输入附件描述" class="layui-textarea">${var.desnames}</textarea></td>
                                            <td>
                                                <select name="file_path" id="file_path_1" lay-verify="" style="height: 32px;" class="layui-input">
                                                    <option value=""></option>
                                                </select>
                                            </td>
                                            <td><input type="text" name="file_type" id="file_type_1" value="${var.file_type}" lay-verify="" autocomplete="off" placeholder="附件类型" class="layui-input"></td>
                                            <td><input type="text" name="file_suffix" id="file_suffix_1" value="${var.file_suffix}" lay-verify="" autocomplete="off" placeholder="附件后缀" class="layui-input"></td>
                                <td style="width:60px;">
...
<script>
    ...
    layui.use(['form', 'layedit', 'laydate','upload'], function(){
        ...
        findCheckboxDictionary('fl1001','div-','read_num','${p.read_num}');
                    findSelectDictionary('12','file_path_1','1');
$.each(${listJson},function(i,n){
                        findSelectDictionary('12','file_path_'+ n.id,'1');
    });
        //自定义验证规则
        ...
        form.on('submit(submit_form)', function(data){
            ...
            $.ajax({
                ...
                url: "${pageContext.request.contextPath}/common/graphicTwxx/update" ,//url
                data: $('#form').serialize(),
    ...
    function delLine(index){
        ...
        $.ajax({
                type: "GET",//方法类型
                dataType: "json",//预期服务器返回的数据类型
                url: "${pageContext.request.contextPath}/common/graphicTwxx/delChild?ids="+id,//url
                data: "",
                success: function (res) {
                    if (res.success) {

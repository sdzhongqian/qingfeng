...
<script>
    ...
    layui.use(['table','form'], function(){
        ...
        table.render({
            ...
            ,url:'${pageContext.request.contextPath}/gencode/mycontent/findListPage?type=${pd.type}'
            ...
            switch(obj.event){
                ...
                case 'del': //删除
                    if(data.length>0){
                        ...
                        $.get("${pageContext.request.contextPath}/gencode/mycontent/del?ids="+ids+"&names="+names,null,function(res){
                                if (res.success) {
        ...
        table.on('tool(gencode_mycontent)', function(obj){
            ...
            if(obj.event === 'info'){
                ...
                layer.confirm('真的要删除数据么?', function(index){
                $.get("${pageContext.request.contextPath}/gencode/mycontent/del?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            reloadData();
                        }else{
...
//            layer.msg('[ID: '+ data.id +'] ' + field + ' 字段更改为：'+ value);
            $.ajax({
                ...
                url: "${pageContext.request.contextPath}/gencode/mycontent/update?"+field+"="+value ,//url
                data: {
                    id:data.id
                },
    ...
    function add(){
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                    content: '${pageContext.request.contextPath}/gencode/mycontent/toAdd?type=${pd.type}',
                area: ['800px', '500px'],
                end: function () {
    ...
    function addMore(){
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                    content: '${pageContext.request.contextPath}/gencode/mycontent/toAddMore?type=${pd.type}',
                area: ['800px', '500px'],
                end: function () {
                    var val = getOpenCloseParam();
                    if(val=="reload"){
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gencode/mycontent/toUpdate?id='+id,
            area: ['800px', '500px'],
            end: function () {
                var val = getOpenCloseParam();
                if(val=="reload"){
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gencode/mycontent/findInfo?id='+id,
            area: ['800px', '500px']
        });
    }

    function updateStatus(id,status){
        $.get("${pageContext.request.contextPath}/gencode/mycontent/updateStatus?id="+id+"&status="+status,null,function(res){
            if (res.success) {
                layer.tips("状态修改成功。");
                reloadData();
            }else{

...
<div class="x-nav">
    ...
    <button class="layui-btn layui-btn-xs" style="margin-left: 10px;" onclick="openAndCloseLeft83('system_organize');"><i title="展开左侧栏" class="iconfont"></i></button>
        ...
        <div class="layui-row layui-col-space1">
            <div class="layui-col-md12">
                <div class="layui-card">
                    ...
                    <div class="layui-card-body ">
                        <table class="layui-hide" id="system_organize" lay-filter="system_organize"></table>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/html" id="barDemo">
    <shiro:hasPermission name="organize:info">
        <a class="layui-btn layui-btn-xs" lay-event="info">详情</a>
    </shiro:hasPermission>
    <shiro:hasPermission name="organize:assignRoleAuth">
        <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="assignRoleAuth">分配权限</a>
    </shiro:hasPermission>
    <jsp:include page="../auth/auth_editdel.jsp">
        <jsp:param value="organize" name="moduleName"/>
    </jsp:include>
    <shiro:hasPermission name="organize:setStatus">
...
<script>
    ...
    layui.use(['table','form'], function(){
        ...
        table.render({
            elem: '#system_organize'
            ,url:'${pageContext.request.contextPath}/system/organize/findListPage'
            ...
            ,cols: [[
                ...
                ,{field:'name', fixed: 'left',title:'组织名称', edit: 'text'}
                ,{field:'short_name', title:'组织简称', edit: 'text'}
        ...
        table.on('toolbar(system_organize)', function(obj){
            ...
            switch(obj.event){
                case 'laytable_tips': //小提示
                    layer.msg("小提示：组织信息模块", {time: 2000});
                    break;
                case 'add': //增加
                    add();
                    break;
        ...
        //监听行工具事件
        table.on('tool(system_organize)', function(obj){
            var data = obj.data;
            //console.log(obj)
            if(obj.event === 'info'){
                info(data.id);
            ...
            } else if(obj.event === 'assignRoleAuth'){
                assignRoleAuth(data.id,'0');
            }
        });

        //监听单元格编辑
        table.on('edit(system_organize)', function(obj){
...
//            layer.msg('[ID: '+ data.id +'] ' + field + ' 字段更改为：'+ value);
            $.ajax({
                ...
                url: "${pageContext.request.contextPath}/system/organize/update?"+field+"="+value ,//url
                data: {
                    id:data.id
                },
    ...
    function add(){
        ...
        var org_cascade = $("#org_cascade").val();
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/organize/toAdd?parent_id='+parent_id+'&name='+name+'&org_cascade='+org_cascade+'&level_num='+level_num,
                area: ['800px', '500px'],
                end: function () {
    ...
    function addMore(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/organize/toAddMore?parent_id='+parent_id+'&name='+name+'&org_cascade='+org_cascade+'&level_num='+level_num,
                area: ['800px', '500px'],
                end: function () {
                    var val = getOpenCloseParam();
                    if(val=="reload"){
    ...
    function exportExcel(){
        ...
        var short_name = $('#short_name').val();
        var status = $("#status option:selected").val();
        window.location.href="${pageContext.request.contextPath}/system/organize/exportData?name="+name
                +"&parent_id="+parent_id
                +"&short_name="+short_name
                +"&status="+status;
    }
    ...
    function updateStatus(id,status){
        $.get("${pageContext.request.contextPath}/system/organize/updateStatus?id="+id+"&status="+status,null,function(res){
            if (res.success) {
                layer.tips("状态修改成功。");
                var parent_id =  $("#tree_id").val();
                reloadData(parent_id);
    ...
    function assignRoleAuth(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/organize/toAssignRoleAuth?id='+id,
            area: ['800px', '500px']
        });
    }

...
<script type="text/html" id="toolbarDemo">
    <div class="layui-btn-container">
        ...
        <shiro:hasPermission name="role:addMore">
...
<script type="text/html" id="barDemo">
    <shiro:hasPermission name="role:info">
        <a class="layui-btn layui-btn-xs" lay-event="info">详情</a>
    </shiro:hasPermission>
    <%--<%@include file="../auth/auth_editdel.jsp"%>--%>


    <shiro:hasPermission name="role:assignAuth">
        <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="assignAuth">分配权限</a>
    </shiro:hasPermission>
    <shiro:hasPermission name="role:setStatus">
                ...
                case 'del': //删除
                    if(data.length>0){
                        ...
                        layer.confirm('删除数据会同步删除【角色菜单信息】，真的要删除数据么?', function(index){
                            ...
                            $.get("${pageContext.request.contextPath}/system/role/del?ids="+ids+"&names="+names,null,function(res){
                                console.log(res);
                                if (res.success) {
...
//            layer.msg('[ID: '+ data.id +'] ' + field + ' 字段更改为：'+ value);
    ...
    function exportExcel(){
        ...
        var short_name = $('#short_name').val();
        var status = $("#status option:selected").val();
        window.location.href="${pageContext.request.contextPath}/system/role/exportData?name="+name
                +"&short_name="+short_name
                +"&status="+status;
    }

...
<div class="x-nav">
    ...
    <button class="layui-btn layui-btn-xs" style="margin-left: 10px;" onclick="openAndCloseLeft83('system_dictionary');"><i title="展开左侧栏" class="iconfont"></i></button>
...
<script type="text/html" id="barDemo">
    ...
    <jsp:include page="../auth/auth_editdel.jsp">
        <jsp:param value="dictionary" name="moduleName"/>
    </jsp:include>
...
<script>
    ...
    layui.use(['table','form'], function(){
        ...
        table.render({
            ...
            ,url:'${pageContext.request.contextPath}/system/dictionary/findListPage'
                ...
                case 'del': //删除
                    if(data.length>0){
                        ...
                        layer.confirm('删除数据会同步删除下级数据，是否进行删除操作?', function(index){
                            ...
                            $.get("${pageContext.request.contextPath}/system/dictionary/del?ids="+ids+"&names="+names,null,function(res){
                                if (res.success) {
        ...
        table.on('tool(system_dictionary)', function(obj){
            ...
            if(obj.event === 'info'){
                ...
                layer.confirm('删除数据会同步删除下级数据，是否进行删除操作?', function(index){
                    $.get("${pageContext.request.contextPath}/system/dictionary/del?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            var parent_id =  $("#tree_id").val();
                            reloadData(parent_id);
...
//            layer.msg('[ID: '+ data.id +'] ' + field + ' 字段更改为：'+ value);
            $.ajax({
                ...
                url: "${pageContext.request.contextPath}/system/dictionary/update?"+field+"="+value ,//url
                data: {
                    id:data.id
                },
    ...
    function add(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/dictionary/toAdd?parent_id='+parent_id+'&name='+name+'&dic_cascade='+dic_cascade+'&level_num='+level_num,
                area: ['800px', '500px'],
                end: function () {
    ...
    function addMore(){
        ...
        if(parent_id==''||parent_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/dictionary/toAddMore?parent_id='+parent_id+'&name='+name+'&dic_cascade='+dic_cascade+'&level_num='+level_num,
                area: ['800px', '500px'],
                end: function () {
                    var val = getOpenCloseParam();
                    if(val=="reload"){
    ...
    function edit(id){
        ...
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/dictionary/toUpdate?id='+id,
            area: ['800px', '500px'],
            end: function () {
                var val = getOpenCloseParam();
                if(val=="reload"){
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/dictionary/findInfo?id='+id,
            area: ['800px', '500px']
        });
    }
    ...
    function exportExcel(){
        ...
        var short_name = $('#short_name').val();
        var status = $("#status option:selected").val();
        window.location.href="${pageContext.request.contextPath}/system/dictionary/exportData?name="+name
                +"&parent_id="+parent_id
                +"&short_name="+short_name
                +"&status="+status;
    }
    ...
    function updateStatus(id,status){
        $.get("${pageContext.request.contextPath}/system/dictionary/updateStatus?id="+id+"&status="+status,null,function(res){
            if (res.success) {
                layer.tips("状态修改成功。");
                var parent_id =  $("#tree_id").val();
                reloadData(parent_id);
